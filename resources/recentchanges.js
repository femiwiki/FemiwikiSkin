$(function() {
  ...
  for (var i = 0; i < elements.length; i++) {
    elements[i].innerHTML =
      '<span class="loading">바뀐글 읽어오는 중...</span>';
  }

  // Fetch data and render
  var url =
    "/w/api.php?action=query&list=recentchanges&rcprop=parsedcomment|title|ids|user|timestamp|sizes|loginfo&rcshow=!bot&rclimit=200&format=json";
  ...
  function renderRow(row) {
    ...
    if (isComment) {
      ...
      url =
        "/w/" +
        encodeTitle(
          nsName.substr(0, nsName.length - 2) + ":" + title + " " + hash
        ) +
        '">' +
        escapeEntity(nsName + ":" + title);
    } else {
    ...
    return (
      ...
      flags.join("\n") +
      "</li>" +
      '<li class="col timestamp"><a href="/index.php?title=' +
      encodeTitle(row.title) +
      '&action=history"><span class="mono">' +
      row.timestampStr +
      "</span> [역사]</a></li>" +
      '<li class="col sizes ' +
      (row.diff > 0 ? "added" : row.diff === 0 ? "" : "deleted") +
      ...
      "</a></li>" +
      '<li class="col title"><a href="' +
      url +
      "</a></li>" +
      '<li class="col parsedcomment">' +
      (row.parsedcomment || "(설명 없음)") +
      "</li>" +
      "</ul>" +
      "</li>"
    );
